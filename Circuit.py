...
def main():
    tester = CircuitCSP("Board: (17, 3)\na: (10, 2)\nb: (5, 2)\nc: (2, 3)\ne: (7, 1)", True, False, True)
    #tester = CircuitCSP("Board: (10, 3)\na: (3, 2)\nb: (5, 2)\ne: (7, 1)", False, True)

    result = tester.backtrack()
    if result:
        print(tester.resultToString(result))
    else:
        print("No valid configuration found.")
    print("Result found: " + str(result))
    print("Calls to backtrack function: " + str(tester.num_b_calls))

    print("Calls to inference function: " + str(tester.num_i_calls))
